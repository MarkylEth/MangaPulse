...
type ReqBody = {
  email?: string;
  mode?: Mode;
  name?: string;       // ник/имя профиля
  password?: string;   // сырой пароль (опционально)
};

// базовый URL для ссылок из письма
...
/**
 * Одноразовая подготовка минимально нужной схемы.
...
async function ensureAuthSchemaOnce() {
  ...
  // запасная старая таблица (если проект её ожидает)
  await query(`
  ...
  // profiles (минимум + уникальный username)
  await query(`
  ...
  // удалим любые старые триггеры на users, которые могли трогать profiles
  await query(`
    DO $$
    DECLARE t record;
    BEGIN
  ...
  // единый триггер-инициатор: при вставке юзера — создать/обновить профиль с username
  await query(`
...
// куда сохраняем одноразовый токен подтверждения
...
export async function POST(req: NextRequest) {
  try {
    ...
    const mode: Mode = body?.mode ?? 'signup';
    ...
    // апсерт пользователя; триггер создаст/обновит профиль
    ...
    const sent = await sendVerificationEmail(email, link, mode as Mode);
    if (!sent.ok) {
