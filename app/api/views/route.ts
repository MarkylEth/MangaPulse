...
function getIp(req: Request): string {
  ...
  return first || h.get('cf-connecting-ip') || h.get('x-real-ip') || '';
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (inc === '1') {
      ...
      await query(
        `create table if not exists page_views(
           slug text not null, day date not null, ip inet not null, ua text, views int not null default 1,
    ...
    return ok({ ok: true, slug, views: rows[0]?.total ?? 0 });
  } catch (e) {
