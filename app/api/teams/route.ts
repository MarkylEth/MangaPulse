...
export async function POST(req: Request) {
  try {
    // ===== AUTH =====
    // cookies() в route handlers синхронный — без await
    const cookie = cookies().get(SESSION_COOKIE)?.value ?? '';

    // никакого .catch — просто await и ловим исключение во внешнем try/catch
    const payload = await verifySession(cookie); // ожидаем, что payload?.sub — id пользователя
