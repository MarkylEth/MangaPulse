...
export default function MessagesPage() {
  const { user, isGuest } = useAuth();        // ← без loading
  ...
  const loadChats = useCallback(async () => {
    try {
      ...
      if (!response.ok) throw new Error(`Не удалось загрузить чаты: ${response.status}`);

      const data = await response.json();
      if (!data.ok) throw new Error(data.message || 'Не удалось загрузить чаты');
  ...
  // Загружаем чаты, когда у нас уже есть авторизованный пользователь
  useEffect(() => {
  ...
  const formatMessageTime = useCallback((dateString: string) => {
    ...
    if (days > 7) return date.toLocaleDateString('ru');
  ...
  const getChatDisplayName = useCallback((chat: Chat) => {
    ...
    return other ? `Чат с пользователем ${other.user_id.slice(0, 8)}` : 'Личные сообщения';
  }, [user?.id]);

  // Спиннер на время загрузки конкретно чатов
  if (loading && user?.id) {
    return (
      ...
      <div className="max-w-4xl mx-auto bg-white dark:bg-slate-800 min-h-screen">
        ...
        <div className="border-b border-gray-200 dark:border-slate-700 p-4">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Сообщения</h1>
        ...
        {error && (
          <div className="p-4 bg-red-50 dark:bg-red-900/20 border-b border-red-200 dark:border-red-800">
            ...
            <button onClick={loadChats} className="text-sm text-red-600 dark:text-red-400 hover:underline mt-1">
              Повторить
            </button>
          </div>
        )}
