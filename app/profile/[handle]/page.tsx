...
/* ================= helpers для работы с API ================= */

/** Аккуратно пробуем несколько URL. 404 считаем «нет такого роута» и идём дальше. */
async function tryJson(urls: string[], init?: RequestInit) {
  for (const url of urls) {
    try {
      const res = await fetch(url, { ...init, credentials: 'include', cache: 'no-store' });
      if (res.status === 404) continue;            // пробуем следующий эндпоинт
      if (!res.ok) continue;                       // другой код → тоже пробуем следующий
      const ct = res.headers.get('content-type') || '';
      if (ct.includes('application/json')) return await res.json();
      // если вдруг вернули не JSON — пропустим как пусто
      return null;
    } catch {
      // сеть отвалилась — пробуем следующий
    }
  }
  return null;
}
...
function normalizeProfile(raw: any, fallbackUsername: string): ProfileLite | null {
  if (!raw) return null;
  const src =
    raw?.data?.profile ?? raw?.data ??
    raw?.profile ??
    (Array.isArray(raw) ? raw[0] : raw);

  if (!src) return null;

  const id = src.id ?? src.user_id ?? src.uuid ?? src.uid;
  if (!id) return null;

  return {
    id: String(id),
    username: src.username ?? fallbackUsername,
    full_name: src.full_name ?? null,
    avatar_url: src.avatar_url ?? null,
    bio: src.bio ?? null,
    created_at: src.created_at ?? null,
    banner_url: src.banner_url ?? null,
    favorite_genres: Array.isArray(src.favorite_genres) ? src.favorite_genres : null,
    telegram: src.telegram ?? null,
    x_url: src.x_url ?? null,
    vk_url: src.vk_url ?? null,
    discord_url: src.discord_url ?? null,
  };
}
        ...
        >
          ...
          >
            ...
            <div className={`relative sticky top-0 z-10 flex items-center justify-between px-6 py-5 border-b backdrop-blur-sm ${
              ...
              <div className="text-xl font-bold">Редактировать профиль</div>
              <button
                disabled={saving}
                onClick={onClose}
                type="button"
            ...
            <form id="editProfileForm" onSubmit={onSubmit} className="px-6 py-6 space-y-8 overflow-y-auto max-h-[calc(100vh-140px)] sm:max-h-[75vh] custom-scrollbar">
                ...
                <div className="flex items-center gap-4">
                  <div className="h-20 w-36 rounded-xl overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 dark:from-slate-600 dark:to-slate-700 shadow-inner ring-2 ring-gray-200 dark:ring-slate-600">
                    {v.banner_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-gray-400 dark:text-slate-400">—</div>
                    )}
                  </div>
                  <input
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  />
                  <div className="absolute bottom-3 right-3 text-xs px-2 py-1 rounded-md bg-gray-100 text-gray-500 dark:bg-slate-700 dark:text-slate-400">
                    {v.bio.length}/300
                  </div>
                </div>
              </div>
                          ...
                          className={`px-3 py-2 rounded-xl text-sm font-medium border-2 transition-all duration-200 ${
                            on
                              ...
                              : disabled 
                                ...
                                : `border-gray-200 dark:border-slate-600 ${theme === 'light' ? 'bg-white text-gray-700 hover:bg-gray-50' : 'bg-slate-700 text-slate-200 hover:bg-slate-600'}`
                          }`}
                        >
                          {g}
                        </button>
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const normalized = normalizeProfile(profRaw, handle);
      ...
      // --- библиотека: идём только если есть id
      if (!normalized.id) {
        setReading([]); setCompleted([]); setFavorites([]); setPlanned([]); setDropped([]);
        setRecentActivity([]);
        setLoading(false);
      ...
      // поддержим несколько вариантов роутов библиотеки
      const libRes = await tryJson([
        ...
        `/api/library/by-user?id=${encodeURIComponent(normalized.id)}`,
        `/api/library/${encodeURIComponent(normalized.id)}`
      ]);
  ...
  const startDM = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/chats', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ type: 'dm', userId: profile.id }),
      });
          ...
          <div className={`rounded-2xl border ${cardBgClass} p-5`}>
            ...
            <div className="flex flex-wrap gap-2">
              {(profile.favorite_genres?.length ? profile.favorite_genres : stats.favoriteGenres).map((genre) => (
                <span
                  key={genre}
                  className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium ring-1 ring-black/5 dark:ring-white/10 shadow-sm ${genreBadgeClass(genre)} ${mode === 'light' ? 'text-gray-900' : 'text-slate-100'}`}
                  title={genre}
                >
                  {genre}
                </span>
